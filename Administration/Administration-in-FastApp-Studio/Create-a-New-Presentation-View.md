...
A user can create a new presentation view to display information within a grid in Profisee FastApp Portal.

To add a new presentation view:

1. In the navigation panel, under Administration, click **Presentation Views**.

   The Presentation Views tab opens.
2. On the Presentation Views toolbar, click **New**.
3. In the **Name** field, enter an identifying name for the new view.
4. Select the desired **Entity** from the list.
5. In the **Description** field, enter an optional description for the new view.
6. Click **Save**.

   The Presentation Views edit panel opens.
7. Complete the information on the **Columns** tab.

   A presentation view must include at least one column (attribute). Name and Code are added by default, but can be removed.

   1. Select the columns to display in an entity control by selecting attributes from the list from the left panel.
      1. Expand a DBA and select one or more attributes beneath it to make a multi-level attribute selection.
      ...
      2. Select individual attributes to add them one at a time.
      3. Click **Clear All** to clear all attribute selections.
      4. Click **Select All** to select all of the attributes.
   2. Arrange the attribute order in the right panel.
      ...
      * Remove attributes from the list by selecting the row and then clicking **Delete**, or by unchecking the selection in the **Available attributes to display** panel.
...
1. Complete the information on the **Filters** tab to build an optional hidden filter.
...
Users cannot change the filter applied to an entity grid control displayed in the browser. It is applied by default as a part of the presentation view and is hidden from users. Users can add additional filters to the entity grid but they cannot alter the hidden filter.

1. Complete the information on the **Sort Columns** tab to build an optional default sort order for the entity grid. You can use an attribute to sort the grid that will not display as a column in the portal page. You can re-sort columns in the entity grid control displayed in the browser.

   1. Select the columns to use to sort the grid by selecting attributes from the list from the left panel.

In general, it is not effective to use more than three attributes to sort the entity grid. You can use more than three attributes if you prefer, however.
...
* Select whether to sort each selected attribute using an ascending or descending order by making selections in the Sort Order column.
* Arrange the sort order by reordering selected attributes in the right panel.
...
1. Configure the optional settings on the **Options** tab.

   1. **Display format**

      Select the way the DBA values display in the entity grid. Possible options include:
      ...
      * Code  
        ...
        *Recommended not to use display format of ‘Name’ (without code) when blank values exist for Name.*
   2. **Show validation column**

      Select to include the column containing member validation status in the entity grid control.
   3. **Use this view as the default view for the entity**
      ...
      If there is only one presentation view for an entity, then that view is automatically the default view for that entity.
