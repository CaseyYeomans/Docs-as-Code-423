...
* The configuration options on this tab are dependent on the selected subscriber. Select each field, and detailed information on completing the field appears under the configuration grid. If an Integrator strategy file is required, it must be created using Profisee ® Integrator (formerly Federation Manager).
