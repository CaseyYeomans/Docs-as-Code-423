...
![Upgrade Projects Architectural Changes](https://Profisee.magentrixcloud.com/sys/staticasset/read/file-i9db0573e3bbf75e7.jpg)

**Version 21R1** introduced architectural changes that require design changes. Customers are encouraged to replace database staging and views in their data integration or ETL processes with ADF templates that use the REST API.  This work effort varies based on your team's familiarity with REST and the volume of entities and source systems (minutes to hours for each pipeline). This design change is mandatory for ProfiseeCloud SaaS customers, as the database is not accessible.

**Version 22R1** introduced architectural changes that require design changes. You must replace reports using iZenda with Power BI reports (hours to days of effort). Re-implement your workflows using the new activities and create webhooks to replace any custom activities (hours to days of effort). Transaction history requires a script be run during the upgrade to convert the transactions to temporal table storage (minutes to hours of effort).

**Version 25R2** introduced a major change in how matching stores the index and manages the processing work, as well as simplification of the options.  Strategies are automatically converted by the software during the upgrade, but you do need to enable them and configure the new processing options with a few minutes of effort.

**Version 25R4** enabled the use of partitioning for history data management and query performance improvements.  The process is automatic, but can take a few minutes to hours to complete depending on data volume.

**Version 26R1** migrated several features from FastApp Studio to Profisee Portal, and their related CLU functions are now managed using the Profisee REST API. Your CICD (continuous integration & deployment) processes need to be adjusted to utilize the [REST API](https://support.profisee.com/wikis/profiseeplatform/profisee_rest_api_overview) to take full advantage of this integration. As additional features are migrated to Profisee Portal, additional CLU functions will be migrated to the Profisee REST API. See the [Release Notes](https://support.profisee.com/wikis/release_notes/profisee_platform_feature_matrix) for further details.
